...
def metal_box(elems, layer=RDD.PLAYER.M6.METAL, margin=0):
    ...
    alias = layer.name
    # return spira.Rectangle(alias=alias, layer=layer, p1=p1, p2=p2)
    return spira.Polygon(alias=alias, shape=box_shape, layer=layer)
...
class I5Contacts(__Junction__):
    ...
    def create_elements(self, elems):
        ...
        #     elems += self.i6
        # elems += MetalBlock().create_elements(elems)
        return elems
...
class Junction(spira.Cell):
    ...
    def create_elements(self, elems):
        ...
        elems += RouteStraight(
            p1=self.i5.ports['R5_e2'],
            p2=self.j5.ports['R5_e2'],
            layer=RDD.PLAYER.R5.ROUTE
        )

        elems += metal_box(elems, layer=RDD.PLAYER.M5.METAL, margin=0.1)
    ...
    def create_ports(self, ports):
        ...
        p0 = self.j5.ports['M6_e1'].unlock
        p1 = self.j5.ports['M6_e3'].unlock
...
if __name__ == '__main__':
    ...
    j1 = Junction()
    j1 = j1.pcell
    s1 = spira.SRef(j1, midpoint=(0, 0), transformation=T)
    cell += s1
