...
def RoutePath(port1, port2, path, width, layer, start_straight=False, end_straight=False):
    """  """
    ...
    if port2.orientation == 180:
        c2 = p2 - [end_straight, 0]
        pts.append(c2)
    pts.append(p2)

    path = gdspy.FlexPath(points=pts, width=width, corners='miter')
    R = Route(shape=path, p1=port1, p2=port2, layer=layer)
    return R
